    ...
    $.fn.extend({
        slider: function (config) {
            ...
            self.html('<div class="slider-range">\n' +
                ...
                '        \n' +
                '    </div><div class="action-value"><span class="act-val">0</span><span>%</span><div>');
            conf.sliderRange = self.find('.slider-range');
            conf.sliderValue = self.find('.act-val');
            ...
            $('#content').css('transform', 'scale(' + conf.initValue / 100 + ')');
            $('#content').css('transform-origin', 'left top');
            //$('.edit_box').width($('.content-box').width()*conf.initValue/100+'px');
            //$('.edit_box').height($('.content-box').height()*conf.initValue/100+'px');
            // 计算并显示显示值的初始位置
            ...
            if (conf.initValue >= conf.minValue && conf.initValue <= conf.maxValue) {
                ...
                conf.sliderValue.text(conf.initValue.toFixed(2));
            ...
            $(document).bind('mousemove', function (ev) {
                ...
                var value = parseInt((conf.minValue +
                    // 先计算出圆的最终位置值
                    (x + conf.sliderHalfWidth)
                ...
                console.log($('.content-box').width());
                $('#content').css('transform', 'scale(' + value / 100 + ')');
                $('#content').css('transform-origin', 'left top');
