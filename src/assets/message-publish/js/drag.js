...
function change_type(e, dom) {
	...
	dbClickNum<=1?timer = setTimeout(()=>{
		if(dbClickNum<=1){
			console.log('change_type')
			_this = dom = $(dom);
			actDomId = dom.prop("id");
			$("#actDomId").attr("actDomId", actDomId);
			let border_arr = dom.css('border') ? dom.css('border').split(' ') : [];
			let border_color = border_arr.slice(2, border_arr.length).join('');
			let type = dom.attr('data-type');
			let fontType = dom.attr('data-ftype');
			showBgOpt(0);
			if (type == 'font' || fontType) {
				...
				let font_color = colorHex(_this.css('color')); //获取当前选中编辑框字体十六进制色值
				let font_bg = colorHex(_this.css('background-color')); //获取当前选中编辑框字体十六进制色值
	...
	if(dbClickNum ==2){
		...
		let newPosition = getPosition(position,dom.id)
...
function init() {
	let max = getUrlParam ('resolutionValue').split('*')
	console.log(max)
		...
		$.extend(document, {
			...
			'call_down': function(e) {
				...
				if (left <= $("#content").width() + 10) {
					let width =  Math.max(30, e.pageX - posix.x + posix.w) / canvasScale
				...
				if (top <= $("#content").height() + 10) {
					let height = Math.max(30, e.pageY - posix.y + posix.h) / canvasScale
