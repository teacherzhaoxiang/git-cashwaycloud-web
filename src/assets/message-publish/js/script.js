(function() {
	...
	! function(e) {
		e.RMenu = {
			init: function(per) {
				if (typeof(per) != "object" && !per.hasOwnProperty('area') && !per.hasOwnProperty('items') && !per.hasOwnProperty(
						'callback')) throw "json 数据错误";
				that = this;
				this.showmenu(per);
				...
				$(per.area).bind('contextmenu', function() {
					var xPos = parseInt(event.pageX + 10)-220;
					var yPos = event.pageY-50;
			...
			contextMenu: function(per, key) {
				...
				html = '<div class="RCM-container RCM-' + key + '" onmouseleave="this.style.display=\'none\'"><ul>';
				$.each(per.items, function(item, val) {
					...
					html += '<li data-content=' + item + ' class="' + center + '">' + icon + '</i>' + val.name + iconAfter +
						'</i>';
