...
export class PublicSFComponent implements OnDestroy, AfterViewInit{
  loading = false;
  ...
  saveFunc(params: Map<string, any>){
    ...
    if (this.saveConfig != null) {
      ...
      if(this.saveConfig['extra']){
        let extra = this.service.handleParams(this.saveConfig['extra'],this.service.list_version.value);
        body = {...extra,...this.sf.value};
      }else {
        body = this.sf.value;
      }
      for(let key in body){
    ...
    this.http.request(method,url,{body:body}).subscribe((res: any) => {
      ...
      if (!res) {
        ...
        if(res['msg']&&res['msg']['errorTerms'].length>0){
          this.message.warning('以下设备'+JSON.stringify(res['msg']['errorTerms'])+'已存在升级任务，未能添加到升级计划');
        }else {
    ...
    this.http.get(componentUri).subscribe((res: any) => {
      ...
      if(res['initUri']){
        if(!this.list_item['item']){
          ...
          this.initUri = environment.atmcManageUrl+this.service.handleUrl(res['initUri'],this.list_item['item']);
      ...
      if (data['ui'] != null) {
        ...
        if (ui['widget'] == 'upload') {
          ...
          data['enum'] = [{
            'name': fileName,
            'status': 'done',
            'url': environment.atmcManageUrl + this.record[key],
        ...
        if (ui['action'] != null) {
          ui['action'] = 'http://192.168.1.29:8090/zuul/file/file' + ui['action'];
        }
