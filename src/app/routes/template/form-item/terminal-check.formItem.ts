  ...
  <sf-item-wrap nz-col nzSpan="24" style="display: inherit;justify-content: space-between;" [id]="id" [schema]="schema" [ui]="ui" [showError]="showError" [error]="error" [showTitle]="schema.title">
    <!-- 开始自定义控件区域 -->
      <div nz-row style="position: relative;z-index: 10">
          ...
          <!--<nz-upload  [nzLimit]="1" [nzShowUploadList]="false" id="import" (nzChange)="uploadChange($event)"
                nzAction={{nzAction}}>
...
export class TerminalCheckFormItemWidget extends ControlWidget implements OnInit {
    /* 用于注册小部件 KEY 值 */
    static readonly KEY = 'terminal-check';
    data:any[] = [];
    dataType:any;
    resultParam:any;
    ...
    constructor(sfComp: SFComponent,private http: _HttpClient,private message: NzMessageService,private modalSrv: NzModalService,cd: ChangeDetectorRef, injector: Injector) {
        super(cd,injector);
    ...
    getCheckTerminal(){
        ...
        const modal = this.modalSrv.create({
            //nzTitle: '选取设备',
            nzContent: TerminalCheckTransferCustomItemComponent,
            nzWidth:800,
            nzClosable:false,
            nzComponentParams:{
    ...
    handleDataResult(){
        ...
        if(this.data !=null && this.data.length>0){
            let title = "";
            console.log("选择设备this.data",this.data);
    ...
    getData(){
        ...
        if(this.dataUrl == null || this.dataUrl == undefined){
            ...
            if(this.dataUrl.split("?").length==2){
                ...
                // sendParams = { params: {typeType:1 }}                //-------------修改到这里----------------------------------
