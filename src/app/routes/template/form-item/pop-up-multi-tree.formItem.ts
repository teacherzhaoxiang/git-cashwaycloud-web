...
@Component({
  ...
  template: `
      <sf-item-wrap style="width: 100%;" [id]="id" [schema]="schema" [ui]="ui" [showError]="showError" [error]="error" [showTitle]="schema.title">
          <multi-tree #multitree [readonly]="readonly" [initUri]="params" [parentComponent]="this" [eventId]="this.eventId" [poPoTree]="true" [layer]="ui.layer"
                [orgId]="orgId" [orgPath]="orgPath" [searchValueTemp]="orgValue">
          </multi-tree>
      </sf-item-wrap>
  `,
  styles:[`
...
export class PopUpMultiTreeFormItemWidget  extends ControlWidget implements OnInit,OnDestroy {
  /* 用于注册小部件 KEY 值 */
  static readonly KEY = 'multi-pop-up-tree';
  readonly:boolean;
  orgValue:any;
  orgId:any;
  orgPath:string;
