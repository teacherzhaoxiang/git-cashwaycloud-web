      ...
      <div class="edit_box" drag>
          ...
          <div (mousedown)="$event.stopPropagation()">
              <div nz-row class="edit_content">
                  <nz-col nzSpan="24">
                      <sf #sf mode="edit" [schema]="editSchema" [formData]="record" button="none" style="width: 100%;"></sf>
                  </nz-col>
              </div>
...
export class UserAddModalComponent {
  ...
  editSchema: any = {
      properties: {
        userName: {type: 'string', title: '操作员账号', maxLength: 36, minLength: 5},
          contact: {type: 'string', title: '操作员名称', maxLength: 20, format: 'regex', pattern: '^[\u4e00-\u9fa5\.\\·]{1,20}$'},
          orgId: {
            type: 'string',
            title: '所属机构',
            ui: {
          ...
          mobile: { type: 'string', title: '手机号码', maxLength: 11, format: 'mobile', },
         // wechatId: {type: 'string', title: '微信号', maxLength: 100, format: 'wechatId', },
          status: { title: '状态', type: 'string',enum: [{label: '失效', value: 0}, {label: '正常', value: 1}], },
      ...
      required: ['userName', 'orgId', 'role', 'mobile','wechatId','status']
  };
  ...
  constructor(private modal: NzModalRef, protected http: _HttpClient, private msg: NzMessageService) {}
    ...
    this.http.post(environment.manage_server_url + '/sys/users?defaultRoleId=' + defaultRoleId, value).subscribe((res: any) => {
        //弹窗保存成功
  ...
  ngOnInit() {
      if(environment['hall-config']) {
          ...
          this.editSchema.properties.contact = {type: 'string', title: '操作员名称', maxLength: 20, format: 'regex', pattern: '^[\u4e00-\u9fa5\.\\·]{1,20}$'};
      }
