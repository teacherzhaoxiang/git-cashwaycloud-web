...
@Component({
    selector: `app-dictonary-type-edit-modal`,
    template: `
...
export class DictonaryTypeEditModalComponent {
    @Input()
    record: any;
    editSchema: any = {
        properties: {

            type: {type: 'string', readOnly: true, title: '类型key', maxLength: 100},
            typeValue: {type: 'string', title: '类型值', maxLength: 100, },
            status: {
        ...
        required: ['type', 'typeValue', 'status', 'language'],
    };
    ...
    save(value: any) {
        this.http.put(environment.manage_server_url + '/sys/dictonarys/' + value.id, value).subscribe((res: any) => {
            //弹窗保存成功
